...
std::function<bool(std::pair<int,double>,std::pair<int,double>)> compare_pair = [](const std::pair<unsigned int, double> &a, const std::pair<unsigned int, double> &b){
    return a.second < b.second;
};

class directed_graph {
    unsigned int get_min_unvisited_node(pair_vector &distances, std::set<unsigned int> &visited);
    std::vector<unsigned int> make_min_path(pair_vector &distances, unsigned int from, unsigned int to);
...
unsigned int directed_graph::get_min_unvisited_node(pair_vector &distances, std::set<unsigned int> &visited){
...
pair_vector directed_graph::dijkstra_util(unsigned int from, unsigned int to){
    pair_vector distances(size, std::make_pair(size + 1, std::numeric_limits<double>::max()));
...
std::vector<unsigned int> directed_graph::make_min_path(pair_vector &distances, unsigned int from, unsigned int to){
...
std::vector<unsigned int> directed_graph::dijkstra(unsigned int from, unsigned int to){
